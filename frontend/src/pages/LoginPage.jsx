...
export default function LoginPage({ setUser }) {
  ...
  useEffect(() => {
    ...
    if (err) setError(`OAuth failed: ${err.replace('_', ' ')}.`);
  }, [location.search]);

  const submit = async (e) => {
    e.preventDefault(); setError(''); setLoading(true);
    try {
      const r = await authService.login(form);
      ...
      <div className="w-full max-w-[400px]">
        ...
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-sm text-[#9a9a9a]">Continue to DarkBot</p>
          </div>
        </div>
        ...
        <p className="mt-8 text-center text-sm text-[#9a9a9a]">
          ...
          <Link to="/signup" className="text-[#19c37d] hover:text-[#20dd8d] underline underline-offset-4 decoration-2 decoration-[#19c37d]/30 font-semibold transition-colors">
            Sign up
          </Link>
        </p>
